...
export const submitAcademicTest = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const physicsTotal = await Question.countDocuments({ 
        subject: { $in: ['Physics'] }
      });
      
      const chemistryTotal = await Question.countDocuments({ 
        subject: { $in: ['Chemistry'] }
      });
      
      const biologyTotal = await Question.countDocuments({ 
        subject: { $in: ['Biology'] }
      });
      ...
      if (totalFromAllCounts === 0) {
        ...
        const physicsQuestions = await Question.find({ 
          subject: 'Physics'
        });
        const chemistryQuestions = await Question.find({ 
          subject: 'Chemistry'
        });
        const biologyQuestions = await Question.find({ 
          subject: 'Biology'
        });
        const socialScienceQuestions = await Question.find({ 
          subject: { $in: ['Social Science', 'History', 'Geography', 'Civics'] }
    ...
    for (let answerIndex = 0; answerIndex < answers.length; answerIndex++) {
      ...
      let subject: 'math' | 'physics' | 'chemistry' | 'biology' | 'socialScience' = 'math';
      ...
      if (answerIndex < 10) {
        ...
        subject = ['math', 'physics', 'chemistry', 'biology', 'socialScience'][extraIndex] as 'math' | 'physics' | 'chemistry' | 'biology' | 'socialScience';
      }
