 ...
 */
export const getSectorDistribution = async (req: Request, res: Response) => {
  try {
    const distribution = await Scholarship.aggregate([
      {
        $group: {
          _id: '$sector',
          count: { $sum: 1 },
    ...
    const withoutSector = await Scholarship.countDocuments({
      $or: [
        { sector: { $exists: false } },
        { sector: null },
        { sector: '' }
 ...
 */
export const updateAllScholarshipSectors = async (req: Request, res: Response) => {
  try {
