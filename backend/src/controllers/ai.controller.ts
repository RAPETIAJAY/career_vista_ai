...
const generateFallbackPrediction = (testResults: any, profile: any) => {
  ...
  Object.entries(interestBoosts).forEach(([stream, keywords]: [string, string[]]) => {
    ...
    rawStreamScores[stream as keyof typeof rawStreamScores] += matchingInterests * 5;
  });
  ...
  const recommendedStream = Object.entries(rawStreamScores).reduce((a, b) => 
    rawStreamScores[a[0] as keyof typeof rawStreamScores] > rawStreamScores[b[0] as keyof typeof rawStreamScores] ? a : b
  )[0];
...
const getSuggestedFields = (scores: any): string[] => {
  const fields: string[] = [];
  const totalScience = (scores.physics || 0) + (scores.chemistry || 0) + (scores.biology || 0);
  if (scores.math + totalScience >= 35) fields.push('Engineering & Technology');
  if (totalScience >= 20) fields.push('Medical & Life Sciences');
  if (scores.math + scores.socialScience >= 35) fields.push('Business & Finance');
  if (scores.socialScience >= 35) fields.push('Humanities & Social Sciences');
