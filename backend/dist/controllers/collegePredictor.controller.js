...
exports.getWhatIfScenarios = exports.compareColleges = exports.getCollegeDetails = exports.predictColleges = void 0;
 ...
 */
async function generateStreamBasedPredictions({ stream, exam, rank, percentile, }) {
    try {
        // Find colleges matching EXACTLY the stream and exam
        let colleges = await College_1.default.find({
            stream: stream,
            exam_accepted: exam
        });
        ...
        if (colleges.length === 0) {
            ...
            colleges = await College_1.default.find({
        ...
        if (colleges.length > 0) {
            ...
            const sampleCollege = await College_1.default.findOne();
            if (sampleCollege) {
            ...
            const allStreams = await College_1.default.distinct('stream');
            const allExams = await College_1.default.distinct('exam_accepted');
        ...
        logger_1.logger.error('Error generating stream-based predictions:', error);
        return [];
    }
}
/**
 ...
 */
function categorizeByRank(userRank, minClosingRank, maxClosingRank) {
 ...
 */
function calculateRankProbability(userRank, minClosingRank, maxClosingRank) {
    const avgClosingRank = (minClosingRank + maxClosingRank) / 2;
    if (userRank <= avgClosingRank * 0.5)
        return 95;
    if (userRank <= avgClosingRank * 0.8)
        return 85;
    if (userRank <= avgClosingRank)
        return 70;
    if (userRank <= avgClosingRank * 1.2)
        return 55;
    if (userRank <= avgClosingRank * 1.5)
 ...
 */
function categorizeByPercentile(userPercentile, closingPercentileRange) {
 ...
 */
function calculatePercentileProbability(userPercentile, minClosingPercentile, maxClosingPercentile) {
    const avgClosingPercentile = (minClosingPercentile + maxClosingPercentile) / 2;
    // For percentiles, higher is better
    if (userPercentile >= avgClosingPercentile * 1.2)
        return 95;
    if (userPercentile >= avgClosingPercentile * 1.1)
        return 85;
    if (userPercentile >= avgClosingPercentile)
        return 70;
    if (userPercentile >= avgClosingPercentile * 0.9)
        return 55;
    if (userPercentile >= avgClosingPercentile * 0.75)
 ...
 */
function parsePercentileRange(percentileStr) {
    try {
        ...
        logger_1.logger.error('Error parsing percentile range:', error);
    }
 ...
 */
function parseRankRange(rankStr) {
    try {
        ...
        logger_1.logger.error('Error parsing rank range:', error);
    }
    return [null, null];
}
/**
...
const predictColleges = async (req, res) => {
    try {
        ...
        const uniqueColleges = new Set(predictions.map((p) => p.college._id.toString()));
...
const getCollegeDetails = async (req, res) => {
    try {
        ...
        const college = await College_1.default.findById(collegeId);
        if (!college) {
            return res.status(404).json({
                success: false,
        ...
        const admissionProbability = calculateAdmissionProbability(user.entranceScores || [], relevantCutoffs, user.category || 'General', user.state || '');
        res.status(200).json({
            success: true,
            college: {
                ...college.toObject(),
        ...
        logger_1.logger.error('Error getting college details:', error);
        res.status(500).json({
            success: false,
...
const compareColleges = async (req, res) => {
    try {
        ...
        try {
            ...
            logger_1.logger.warn('AI comparison summary failed:', error);
    ...
    try {
        ...
        for (const scenario of scenarios) {
            ...
            results.push({
                ...
                entranceScores: scenario.entranceScores,
                predictions: {
                    ambitious: predictions?.filter((p) => p.category === 'Ambitious').slice(0, 3) || [],
                    moderate: predictions?.filter((p) => p.category === 'Moderate').slice(0, 3) || [],
                    safe: predictions?.filter((p) => p.category === 'Safe').slice(0, 3) || [],
                },
                summary: {
        ...
        logger_1.logger.error('Error generating what-if scenarios:', error);
        res.status(500).json({
            success: false,
...
exports.getWhatIfScenarios = getWhatIfScenarios;
// Helper functions
const generateCollegePredictions = async (input) => {
    ...
    try {
        ...
        let aiUsed = true;
        const aiPredictions = await (0, ai_1.getCollegePredictions)({
            entranceScores,
        ...
        if (aiPredictions.length > 0) {
            aiPredictions.aiSource = 'ai';
            return aiPredictions;
        }
    }
    catch (error) {
        logger_1.logger.warn('AI predictions failed, using database fallback:', error);
    }
    // Fallback to database-based predictions
    const db = await generateDatabasePredictions(user, entranceScores, preferences);
    db.aiSource = 'fallback';
    return db;
};
const generateDatabasePredictions = async (user, entranceScores, preferences) => {
    const predictions = [];
    ...
    for (const college of colleges) {
        const relevantCutoffs = getRelevantCutoffs(college, user);
        const probability = calculateAdmissionProbability(entranceScores, relevantCutoffs, user.category || 'General', user.state || '');
        if (probability > 0) {
            let category = 'Safe';
            if (probability < 40)
                category = 'Ambitious';
...
const getRelevantCutoffs = (college, user) => {
    if (!college.cutoffs)
        return [];
    return college.cutoffs.filter((cutoff) => {
        // Filter by category
        if (cutoff.category && cutoff.category !== user.category)
            return false;
        // Filter by state (for state quota)
        if (cutoff.quota === 'State' && cutoff.state !== user.state)
            return false;
        return true;
    });
};
const calculateAdmissionProbability = (entranceScores, cutoffs, category, state) => {
    if (!entranceScores.length || !cutoffs.length)
        return 0;
...
const getRecommendedCourses = (college, user) => {
    ...
    return courses
        ...
        const aMatch = userInterests.some((interest) => a.name.toLowerCase().includes(interest.toLowerCase()));
        const bMatch = userInterests.some((interest) => b.name.toLowerCase().includes(interest.toLowerCase()));
        if (aMatch && !bMatch)
            return -1;
        if (!aMatch && bMatch)
            return 1;
...
const getBestCourse = (college, entranceScores) => {
    const courses = college.courses || [];
    if (courses.length === 0)
        return 'General';
    ...
    if (examNames.includes('JEE Main') || examNames.includes('JEE Advanced')) {
        return courses.find((c) => c.category === 'Engineering')?.name || courses[0]?.name;
    }
    if (examNames.includes('NEET')) {
        return courses.find((c) => c.category === 'Medical')?.name || courses[0]?.name;
    }
    return courses[0]?.name || 'General';
};
const calculateROI = (college) => {
...
const generatePros = (college, user) => {
    const pros = [];
    if (college.accreditation?.naac === 'A++')
        pros.push('NAAC A++ accredited');
    if (college.placement?.placementRate > 80)
        pros.push('High placement rate');
    if (college.fees?.tuition < 200000)
        pros.push('Affordable fees');
    if (college.location?.state === user.state)
        pros.push('Home state advantage');
    if (college.type === 'Government')
        pros.push('Government institution');
    return pros.slice(0, 3);
};
const generateCons = (college, user) => {
    const cons = [];
    if (college.fees?.tuition > 500000)
        cons.push('High fees');
    if (college.placement?.placementRate < 60)
        cons.push('Lower placement rate');
    if (college.location?.state !== user.state)
        cons.push('Out of state');
    if (!college.accreditation?.naac)
        cons.push('No NAAC accreditation');
    return cons.slice(0, 3);
};
const analyzeEntranceScores = (entranceScores) => {
    const analysis = {
...
const generateComparisonSummary = async (comparison, user) => {
    // This would use AI to generate a summary
    // For now, return a simple summary
    const bestROI = comparison.reduce((best, current) => current.roi > best.roi ? current : best);
    const mostAffordable = comparison.reduce((best, current) => current.fees.total < best.fees.total ? current : best);
...
const generateComparisonRecommendations = (comparison) => {
    return [
        {
            type: 'Best ROI',
            college: comparison.reduce((best, current) => current.roi > best.roi ? current : best).college,
        },
        {
            type: 'Most Affordable',
            college: comparison.reduce((best, current) => current.fees.total < best.fees.total ? current : best).college,
        },
        {
            type: 'Highest Probability',
            college: comparison.reduce((best, current) => current.admissionProbability > best.admissionProbability ? current : best).college,
        },
    ];
};
const generateScenarioInsights = (scenarios) => {
    const insights = [];
