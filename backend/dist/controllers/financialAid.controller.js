...
exports.exportFinancialReport = exports.generateFinancialReport = exports.getScholarshipDetails = exports.calculateLoanEMI = exports.getFinancialAidRecommendations = void 0;
 ...
 */
const getFinancialAidRecommendations = async (req, res) => {
    try {
        ...
        const maxLoanAmount = Math.max(...loans.map((l) => l.amount || 0), 0);
        res.status(200).json({
            success: true,
            financialAid: {
        ...
        logger_1.logger.error('Error getting financial aid recommendations:', error);
        res.status(500).json({
            success: false,
...
exports.getFinancialAidRecommendations = getFinancialAidRecommendations;
/**
...
const calculateLoanEMI = async (req, res) => {
    try {
        ...
        if (loanType) {
            loanDetails = await Loan_1.default.findOne({ name: new RegExp(loanType, 'i') });
        }
        res.status(200).json({
            success: true,
            calculation: {
        ...
        logger_1.logger.error('Error calculating loan EMI:', error);
        res.status(500).json({
            success: false,
...
exports.calculateLoanEMI = calculateLoanEMI;
/**
...
const getScholarshipDetails = async (req, res) => {
    try {
        ...
        const userId = req.userId;
        const scholarship = await Scholarship_1.default.findById(scholarshipId);
        if (!scholarship) {
            return res.status(404).json({
                success: false,
        ...
        logger_1.logger.error('Error getting scholarship details:', error);
        res.status(500).json({
            success: false,
...
exports.getScholarshipDetails = getScholarshipDetails;
/**
...
const generateFinancialReport = async (req, res) => {
    try {
        ...
        for (const college of colleges) {
            ...
            const applicableScholarships = scholarships.filter(s => isScholarshipApplicableToCollege(s, college));
            const scholarshipAmount = applicableScholarships.reduce((sum, s) => sum + (typeof s.amount === 'number' ? s.amount : s.amount?.min || 0), 0);
...
const exportFinancialReport = async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error exporting financial report:', error);
        res.status(500).json({
            success: false,
...
// Helper functions
const getMatchingScholarships = async (user) => {
    const query = {
        $and: [
            {
                $or: [
    ...
    const scholarships = await Scholarship_1.default.find(query)
...
const getSuitableLoans = async (user) => {
...
const calculateScholarshipMatchScore = (scholarship, user) => {
    ...
    if (scholarship.amount > 50000)
        score += 10;
    if (scholarship.amount > 100000)
        score += 5;
    return Math.min(score, 100);
};
const calculateLoanSuitabilityScore = (loan, user) => {
    let score = 50; // Base score
    // Lower interest rate = higher score
    if (loan.interestRate < 8)
        score += 20;
    else if (loan.interestRate < 10)
        score += 10;
    // Higher max amount = higher score
    if (loan.maxAmount > 2000000)
        score += 15;
    else if (loan.maxAmount > 1000000)
        score += 10;
    // Category benefits
...
const checkScholarshipEligibility = (scholarship, user) => {
    const eligibility = {
        eligible: true,
        reasons: [],
        requirements: [],
    ...
    if (scholarship.eligibility?.category &&
        ...
        eligibility.reasons.push(`Category: Required ${scholarship.eligibility.category.join(' or ')}, you are ${user.category}`);
    }
    // Check state
    if (scholarship.eligibility?.state &&
        ...
        eligibility.eligible = false;
        eligibility.reasons.push(`State: Required ${scholarship.eligibility.state.join(' or ')}, you are from ${user.state}`);
    }
    // Check income
    if (user.income && scholarship.eligibility?.maxIncome &&
...
const getApplicationTimeline = (scholarship) => {
    const timeline = [];
...
const getApplicationSteps = (scholarship) => {
    return [
...
const getRequiredDocuments = (scholarship) => {
    const commonDocs = [
        'Aadhaar Card',
        'Income Certificate',
...
const isScholarshipApplicableToCollege = (scholarship, college) => {
...
const calculateEducationROI = (college, netCost) => {
    const avgPackage = college.placement?.averagePackage || 0;
    if (netCost === 0)
        return Infinity;
    return Math.round((avgPackage / netCost) * 100);
};
const generateFinancialRecommendations = (report, user) => {
    const recommendations = [];
    // Best value college
    const bestValue = report.colleges.reduce((best, current) => current.roi > best.roi ? current : best);
    ...
    const mostAffordable = report.colleges.reduce((best, current) => current.netCost < best.netCost ? current : best);
    recommendations.push({
