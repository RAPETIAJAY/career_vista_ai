...
exports.getCareerOptions = exports.getStreamRecommendations = void 0;
 ...
 */
const getStreamRecommendations = async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting stream recommendations:', error);
        res.status(500).json({
            success: false,
...
exports.getStreamRecommendations = getStreamRecommendations;
/**
 * Get career options for a specific stream
 */
const getCareerOptions = async (req, res) => {
    try {
        ...
        logger_1.logger.error('Error getting career options:', error);
        res.status(500).json({
            success: false,
...
exports.getCareerOptions = getCareerOptions;
/**
 ...
 */
const generateRecommendations = (subjects, interests) => {
    const recommendations = [];
    // Science stream
    const totalScience = (subjects.physics || 0) + (subjects.chemistry || 0) + (subjects.biology || 0);
    const avgScience = totalScience / 3;
    if (subjects.math >= 70 && avgScience >= 70) {
        let confidence = (subjects.math + avgScience) / 2;
    ...
    // Commerce stream
    if (subjects.math >= 60 && subjects.socialScience >= 60) {
        let confidence = (subjects.math + subjects.socialScience) / 2;
        // Adjust confidence based on interests
        if (interests.some(interest => [
            'business',
            'economics',
    ...
    // Arts stream
    if (subjects.socialScience >= 70) {
        let confidence = subjects.socialScience;
        // Adjust confidence based on interests
        if (interests.some(interest => [
            'literature',
            'history',
 ...
 */
const getCareerOptionsForStream = (stream) => {
    switch (stream) {
        case 'Science':
        case 'MPC':
            return [
