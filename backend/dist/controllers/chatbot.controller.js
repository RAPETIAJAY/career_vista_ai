...
exports.getCounselorSuggestions = exports.getPersonalizedRecommendation = exports.chatWithCounselorBot = void 0;
...
const getSystemPrompt = (userClass = '12', stream, hasTestResults = false, hasProfile = false) => {
...
const chatWithCounselorBot = async (req, res) => {
    try {
        ...
        if (userId) {
            try {
                userData = await User_1.default.findById(userId);
                hasTestResults = !!(userData?.examCompleted && userData?.testScores?.fundamentals);
                hasProfile = !!userData?.profileCompleted;
            }
            catch (error) {
                logger_1.logger.warn('Could not fetch user data for chat context:', error);
            }
        }
        ...
        if (userData && (hasTestResults || hasProfile)) {
            ...
            if (hasProfile) {
                ...
                if (userData.state)
                    contextMessage += `State: ${userData.state}\n`;
                if (userData.category)
                    contextMessage += `Category: ${userData.category}\n`;
            }
            if (hasTestResults) {
                const testResults = userData.testScores.fundamentals;
                const subjects = testResults.subjects;
                contextMessage += `Test Results: Total ${testResults.total}/100, Math: ${subjects.math}/10, Physics: ${subjects.physics}/10, Chemistry: ${subjects.chemistry}/10, Biology: ${subjects.biology}/10, Social Science: ${subjects.socialScience}/10\n`;
                if (testResults.strengths?.length)
                    contextMessage += `Strengths: ${testResults.strengths.join(', ')}\n`;
                if (testResults.weaknesses?.length)
                    contextMessage += `Weaknesses: ${testResults.weaknesses.join(', ')}\n`;
            }
        ...
        logger_1.logger.info('🤖 Career counselor chat request:', {
            userId,
        ...
        const models = [
            ...
            'neversleep/noromaid-mixtral-8x7b-instruct:free', // Noromaid Mixtral
            'gryphe/mythomax-l2-13b:free', // MythoMax L2 13B
            'undi95/toppy-m-7b:free', // Toppy M 7B
            'jondurbin/airoboros-l2-70b-gpt4-1.4.1:free', // Airoboros L2 70B
            'meta-llama/codellama-34b-instruct:free', // CodeLlama 34B
            'phind/phind-codellama-34b:free', // Phind CodeLlama 34B
            'huggingface/zephyr-7b-alpha:free', // Zephyr 7B Alpha
            'open-orca/mistral-7b-openorca:free', // Mistral 7B OpenOrca
            'lizpreciatior/lzlv-70b-fp16-hf:free', // LZLV 70B
            'alpindale/goliath-120b:free', // Goliath 120B
            'neversleep/noromaid-20b:free', // Noromaid 20B
            'gryphe/mythalion-13b:free', // Mythalion 13B
            'mancer/weaver:free', // Weaver
            'pygmalionai/mythalion-13b:free', // Pygmalion Mythalion 13B
            'koboldai/psyfighter-13b:free', // PsyFighter 13B
            // Smaller but reliable models
            'microsoft/wizardlm-2-8x22b:free', // WizardLM 2 8x22B
            'microsoft/wizardlm-2-7b:free', // WizardLM 2 7B
            'google/gemma-2b-it:free', // Gemma 2B (very fast)
            'microsoft/phi-2:free', // Phi-2 (lightweight)
            'microsoft/phi-1_5:free', // Phi-1.5 (ultra fast)
            'togethercomputer/redpajama-incite-7b-chat:free', // RedPajama 7B
            'togethercomputer/redpajama-incite-3b-chat:free', // RedPajama 3B
            'huggingface/starcoder:free', // StarCoder (code-focused)
            'bigcode/starcoder:free', // BigCode StarCoder
            'cerebras/btlm-3b-8k-chat:free', // BTLM 3B 8K
            // Fallback options
            'anthropic/claude-3-haiku:free', // Claude 3 Haiku (if available)
            'openai/gpt-3.5-turbo:free', // GPT-3.5 Turbo (if available)
            'cohere/command-r:free', // Command R (if available)
            'ai21/jamba-instruct:free' // Jamba Instruct (if available)
        ];
        let response;
        let lastError;
        for (const model of models) {
            try {
                logger_1.logger.info(`🤖 Trying model: ${model}`);
                response = await axios_1.default.post(`${OPENROUTER_BASE_URL}/chat/completions`, {
                    model: model,
                    messages: messages,
                    max_tokens: 800,
                    temperature: 0.7,
                ...
                lastError = error;
                logger_1.logger.warn(`❌ Model ${model} failed:`, error.response?.status || error.message);
        ...
        logger_1.logger.info('✅ Career counselor response generated successfully');
        res.status(200).json({
            success: true,
            data: {
                message: aiResponse,
        ...
        logger_1.logger.error('❌ Career counselor chat error:', error);
...
exports.chatWithCounselorBot = chatWithCounselorBot;
...
- Mathematics: ${testResults.subjects.math}/10 (${Math.round((testResults.subjects.math / 10) * 100)}%)
- Physics: ${testResults.subjects.physics}/10 (${Math.round((testResults.subjects.physics / 10) * 100)}%)
- Chemistry: ${testResults.subjects.chemistry}/10 (${Math.round((testResults.subjects.chemistry / 10) * 100)}%)
- Biology: ${testResults.subjects.biology}/10 (${Math.round((testResults.subjects.biology / 10) * 100)}%)
- Social Science: ${testResults.subjects.socialScience}/10 (${Math.round((testResults.subjects.socialScience / 10) * 100)}%)
...
I'm here to help you make informed decisions about your academic future! 🚀`;
        ...
        logger_1.logger.error('❌ Error getting personalized recommendation:', error);
        res.status(500).json({
            success: false,
...
exports.getPersonalizedRecommendation = getPersonalizedRecommendation;
const getCounselorSuggestions = async (req, res) => {
    try {
        const { userClass = '12', stream, interests = [] } = req.query;
        // Generate contextual conversation starters
        const suggestions = generateConversationStarters(userClass, stream, interests);
        res.status(200).json({
            success: true,
            data: {
                suggestions,
                welcomeMessage: getWelcomeMessage(userClass, stream)
            }
        });
    }
    catch (error) {
        logger_1.logger.error('❌ Error getting counselor suggestions:', error);
        res.status(500).json({
            success: false,
...
exports.getCounselorSuggestions = getCounselorSuggestions;
const generateConversationStarters = (userClass, stream, interests = []) => {
...
const getWelcomeMessage = (userClass, stream) => {
